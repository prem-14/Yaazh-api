const { pgTableColumns } = require('../../../common/columns')
const {
  generateColumns,
  generateInsertData,
  generateQuery,
  generateUpdateData,
} = require('../../../common/commonFunctions')
const AppError = require('../../../common/error')

exports.addNewBanner = async (req, next) => {
  req.body.image = JSON.stringify(req.body.image)
  const bannerConfig = generateColumns(pgTableColumns['b_c'])

  const { columns, exp, values } = generateInsertData(bannerConfig, req.body)

  await _pgPool.query(
    `
    INSERT INTO "banner" (${columns}) VALUES (${exp});
    `,
    [...values]
  )
}

exports.getAllBanners = async (req) => {
  const { columns, alias } = generateColumns(pgTableColumns['b'])

  const recordsQuery = generateQuery(req.body, 0)
  const totalRecordsQuery = generateQuery(req.body, 1)

  const { rows: records } = await _pgPool.query(`
        SELECT ${columns} FROM ${alias} WHERE ${recordsQuery}
    `)

  const { rows: totalRecords } = await _pgPool.query(`
       SELECT count(*) total_count FROM banner  WHERE ${totalRecordsQuery}
    `)

  return { records, totalRecords: totalRecords[0].total_count }
}

exports.getBanner = async (req) => {
  const { rows: records } = await _pgPool.query(
    `
          SELECT * FROM banner WHERE id = $1
      `,
    [req.query.id]
  )

  return { records }
}

exports.updateBanner = async (req) => {
  const bannerConfig = generateColumns(pgTableColumns['b_c'])

  const { columns, values, lastIndex: i } = generateUpdateData(bannerConfig, req.body)
  const { id } = req.body

  const { rows: records } = await _pgPool.query(
    `
      UPDATE banner SET ${columns} where "id" = $${i + 1}
        `,
    [...values, id]
  )

  return { records }
}

exports.deleteBanner = async (req) => {
  const { id } = req.body

  const { rows: records } = await _pgPool.query(
    `
      DELETE FROM banner where id = $1 ;
      `,
    [id]
  )

  return { records }
}

exports.changeBannerStatus = async (req) => {
  const { id, active } = req.body

  const { rows: records } = await _pgPool.query(
    `
      UPDATE banner set active = $1 where id = ANY($2) ;
      `,
    [active, id]
  )

  return { records }
}
